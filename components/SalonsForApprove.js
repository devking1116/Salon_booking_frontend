...
class SalonsForApprove extends Component {

  constructor (props) {
    super(props)
    const filters = this.makeFilters(props.unapprsalons)
    this.state = {
  ...
  makeFilters (unapprsalons) {
    console.log('makeFilters', unapprsalons)
    const areaFilters = []
    const zipFilters = []
    const serviceFilters = [] //TODO add serviceFilters and make this functional
    const aFilters = []
    const zFilters = []
    for (const salonId in unapprsalons) {
      const salon = unapprsalons[salonId]
  ...
  componentWillReceiveProps (nextProps) {
    const filters = this.makeFilters(nextProps.unapprsalons)
  ...
  render () {
    const dataSource = _.values(this.props.unapprsalons)
    return (
...
SalonsForApprove.propTypes = {
  dispatch: PropTypes.func.isRequired,
  unapprsalons: PropTypes.object.isRequired
}

const mapStateToProps = (state) => (
  {
    unapprsalons: state.unapprsalons
  })

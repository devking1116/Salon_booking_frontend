...
import { addSalon, updateSalon, addUnapprovedSalon, updateUnapprovedSalon } from './SalonActions'
import { addPack } from './PackActions'
import { ref, auth } from '../constants/firebase'
import { fetchWaitList } from '../controllers/WaitList'
import { fetchRequests } from '../controllers/Requests'
import { fetchAreas } from '../controllers/Areas'
...
function * fetchUnapprovedSalons (dispatch) {
  console.log('fetch unapproved salons')
  try {
    ref.child('salonsForApprove').on('child_added', function (snapshot, prevChildKey) {
      const salon = snapshot.val()
      salon.id = snapshot.key
      dispatch(addUnapprovedSalon(salon))
    })
    ref.child('salonsForApprove').on('child_changed', function (snapshot, prevChildKey) {
      const salon = snapshot.val()
      salon.id = snapshot.key
      dispatch(updateUnapprovedSalon(salon))
    })
  } catch (e) {
    console.error(e)
  }
...
export function init () {
  return function * (dispatch, getState) {
    try {
      ...
      yield fetchUnapprovedSalons(dispatch)
      yield fetchPacks(dispatch)
      yield fetchWaitList(dispatch)
      yield fetchAreas(dispatch)
